...
async def dispatch_daily_tasks():
    ...
    for row in profiles:
        ...
        # Task regeneration is handled elsewhere (task_engine)
...
async def reset_user_state(user_id: int):
    ...
    for row in profiles:
        ...
        cur.execute(
            "DELETE FROM assigned_tasks WHERE profile_id = ?",
            (profile_id,),
        )
        cur.execute(
            "DELETE FROM task_history WHERE profile_id = ?",
            (profile_id,),
        )
        cur.execute(
            "DELETE FROM boss_history WHERE profile_id = ?",
            (profile_id,),
        )
